"""
Dialog zum Bearbeiten von Veranstaltungen
"""
...
class VeranstaltungBearbeitenDialog(ctk.CTkToplevel):
    """Dialog zum Bearbeiten einer Veranstaltung"""

    def __init__(self, parent, veranstaltung, data_manager):
        super().__init__(parent)
        self.data_manager = data_manager
        self.veranstaltung = veranstaltung
        self.result = None

        self.title("Veranstaltung bearbeiten")
    ...
    # -----------------------------------------------------
    def _load_data(self):
        """Lädt Veranstaltungsdaten in Felder"""
        self.name_entry.insert(0, self.veranstaltung.name)
        self.von_datum_input.set_date(self.veranstaltung.von_datum)
        self.bis_datum_input.set_date(self.veranstaltung.bis_datum)

        if self.veranstaltung.ort:
            self.ort_entry.insert(0, self.veranstaltung.ort)

        if self.veranstaltung.beschreibung:
            self.beschreibung_text.insert("1.0", self.veranstaltung.beschreibung)

        if self.veranstaltung.max_teilnehmer:
            self.max_teilnehmer_entry.insert(0, str(self.veranstaltung.max_teilnehmer))
