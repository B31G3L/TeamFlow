"""
Dialog zum Bearbeiten von Feiertagen
"""
...
class FeiertagBearbeitenDialog(ctk.CTkToplevel):
    """Dialog zum Bearbeiten eines Feiertags"""

    def __init__(self, parent, feiertag, data_manager):
        super().__init__(parent)
        self.data_manager = data_manager
        self.feiertag = feiertag
        self.result = None

        self.title("Feiertag bearbeiten")
    ...
    # -----------------------------------------------------
    def _load_data(self):
        """Lädt Feiertagsdaten in Felder"""
        self.name_entry.insert(0, self.feiertag.name)
        self.datum_input.set_date(self.feiertag.datum)

        if self.feiertag.bundesland:
            self.bundesland_var.set(self.feiertag.bundesland)
        else:
            self.bundesland_var.set("Bundesweit")

        self.aktiv_var.set(self.feiertag.aktiv)
