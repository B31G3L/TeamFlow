...
// ========================================
// IPC-HANDLER FÜR MITARBEITER-JAHRES EXCEL-EXPORT
// Ans Ende von main.js anhängen (vor dem letzten Semikolon / EOF)
// ========================================

ipcMain.handle('export:employeeYearExcel', async (event, data) => {
  logger.info('📊 Mitarbeiter-Jahres-Excel-Export gestartet', {
    employee: data.employee?.name,
    jahr: data.jahr
  });
  ...
  try {
    ...
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, -5);
    const employeeName = (data.employee?.name || 'Mitarbeiter').replace(/[^a-zA-Z0-9]/g, '_');
    const tempJsonPath = path.join(exportDir, `temp_year_excel_${timestamp}.json`);
    const outputPath = path.join(
      exportDir,
      `Jahresuebersicht_${employeeName}_${data.jahr}_${timestamp}.xlsx`
    );
    ...
    const scriptDir = isDev
      ? path.join(__dirname, 'scripts')
      : path.join(process.resourcesPath, 'scripts');
    const scriptPath = path.join(scriptDir, 'export_employee_year_excel.py');
    ...
    const result = await new Promise((resolve) => {
      ...
      child.on('close', (code) => {
        if (code === 0) {
          logger.success('✅ Jahres-Excel erstellt', { path: outputPath });
          resolve({ success: true, path: outputPath });
        } else {
          logger.error('❌ Export fehlgeschlagen', { code, stderr });
          resolve({ success: false, error: `Exit Code ${code}: ${stderr}` });
    ...
    logger.error('❌ Jahres-Excel-Export fehlgeschlagen', { error: error.message });
    return { success: false, error: error.message };
  }
});